...
class ChatList extends StatefulWidget {
  ...
  /// If to animate the item insertion and removal.
  ...
  /// Reverse or not. For TV with arrow buttons, it can only be the non-reverse
  /// order.
  final bool reverse;
...
class _ChatListState extends State<ChatList>
  ...
  void _calculateAndAnimateDiffs(List<Object> oldList) {
    ...
    for (final update in diffResult.getUpdates(batch: true)) {
      update.when(
        insert: (pos, count) {
          if (count <= 1) {
            ...
            // Normal animation for the last item.
            _listKey.currentState?.insertItem(pos + count - 1);
          }
        },
        remove: (pos, count) {
          for (var i = 0; i < count; i++) {
            final item = oldList[pos + i];
  ...
  void _calculateDiffs(List<Object> oldList) {
    if (widget.animateChange) {
      _calculateAndAnimateDiffs(oldList);
    }
  ...
  Widget get _sliver {
    ...
    return SliverList(
      ...
      delegate: SliverChildBuilderDelegate(
        (context, index) => _newMessageBuilder(index, null),
